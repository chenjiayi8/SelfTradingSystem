...
def clickText(Application, text):
    ...
    x, y = getTextMidPoint(tabImg, text=text, fontSize=18)
    app_top_window.set_focus()
    time.sleep(2)
...
def getOperator(Application):
    ...
    #0 担保物买入 1 担保物卖出 2 融资买入
    bid_obj.wait('visible',timeout=30).set_focus().select(2)
    time.sleep(3)
    ...
    # button_selectAll.wait('visible',timeout=30).set_focus().click()
    button_send = app_top_window['发送Button']
    # button_send.wait('visible',timeout=30).set_focus().click()
    ...
    operator = {'bid_obj': bid_obj, 'edit_code': edit_code,
                ...
                'button_send':button_send, 'pid': pid, 
                }
    return app, operator

def getFinanceData(Application):
    app = Application(backend="win32")
    app.connect(path=exePath)
    app_top_window = app.top_window()
    app_top_window.maximize()
    ...
    # app_top_window.set_focus()
    time.sleep(5)
    app.Dialog['确定Button'].wait('visible',timeout=120).set_focus().click()
    

    time.sleep(5)
    app_top_window = app.top_window()
    app_top_window.set_focus()
    pyautogui.moveTo(584, 211)#全部选中
    pyautogui.click()
    time.sleep(5)
    pyautogui.moveTo(400, 169)#延期申请
    pyautogui.click()
    time.sleep(5)
    ...
    time.sleep(2)
    financeData = {}
    financeData['totalValue'] = total_value
    financeData['cash'] = cash
    financeData['debit'] = debit
    financeData['credit'] = credit 
    financeData['credit_cash'] = credit_cash
    return financeData
...
def login(Application):
    ...
    app_top_window.set_focus()
    time.sleep(5)
    app.Dialog['交易密码Edit'].wait('visible',timeout=120).type_keys(os.environ['HuataiPass'])
    app.Dialog['核新加密Edit'].wait('visible',timeout=120).type_keys(os.environ['HuataiComPass'])
    app.Dialog['用户登录Button'].click()
    time.sleep(60)
   
    app.营业部公告Dialog['确定Button'].wait('visible',timeout=120).set_focus().click()
    
    # app_top_window.print_control_identifiers()#inspect
    financeData = getFinanceData(Application)
    app, operator = getOperator(Application)
    operator.update(financeData)
    return app, operator
...
def processTasks(sysObj, ordersTable, credit_account):
    ...
    for i in range(len(ordersTable)):
        if type(ordersTable['Name'][i]) is str:
            ...
            task['market'] = '上海' if int(ordersTable['Code'][i]) > 500000 else '深圳'
...
def makeorders(sysObj, app, operator, tasks, killAtExit=True):
    app_top_window_rect_max = app.top_window().rectangle()
    ...
    for task in tasks:
        ...
        time.sleep(2)
        chooseMarket(app_top_window_rect_max, app, task)
...
def loginN(Application, loopguard = 5):
    if loopguard > 0:
        try:
            ...
            print("Failed to login once, tried another time")
            loopguard -= 1
            if loopguard <= 0:
                msg = "Failed to login for N times"
                sendEmail('Alert from {}'.format(get_host_name()), msg, 'chenjiayi_344@hotmail.com')
                raise(msg)
            return loginN(Application, loopguard)
...
def loginAndOrder(sysObj): 
    ...
    try:
        ...
        operator['orderedTable'] = orderedTable2
        if len(tasks) > 0:
            sysObj.write_log("Ready for making orders in ")
            sysObj, exitCode = makeorders(sysObj, app, operator, tasks)
            printTable(orderedTable)
        else:
            sysObj.write_log("No need to make orders")
            killApp(sysObj.pywinauto_app)
        ...
        sysObj.write_log ("Need assisstance for unexpected error:\n {}".format(sys.exc_info()))
...
def chooseMarket(app_top_window_rect_max, app, task):
    if app_top_window_rect_max != app.top_window().rectangle():
        marketDiag = app['请选择证券Dialog'].wrapper_object()
        children = marketDiag.children()
        targetCheck = [task['market'] in child.texts()[0] for child in children]
        if any(targetCheck):
            child = children[targetCheck.index(True)]
            child.set_focus().click()
            time.sleep(2)
...
if __name__ == "__main__":
    ...
    # task['name'] = '创业板ETF周' 
    # task['type'] = '目标市值两融'
    # task['code'] = numberToStr(159967)
    # task['market'] = '上海' if 159967 > 500000 else '深圳'
    # task['tradeCode'] = int(2)
    # task['price'] = round(0.675, 3)
    # task['amount'] = int(500)
    # app_top_window = app.top_window()
    # app_top_window.set_focus()
    # app_top_window_rect_max = app_top_window.rectangle()
    ...
    # operator['edit_amount'].wait('visible',timeout=30).set_focus().set_edit_text(amountStr)
    # time.sleep(2)
    # operator = getOperator(Application)
    # time.sleep(5)
    # app, operator = login(Application)
    # _, orderedTable = getDFFromDB(sysObj.db_sql, 'Ordered', 0, 9)
    # orderedTable['TradeCode'] = orderedTable['TradeCode'].apply(float)
    ...
    # sysObj.write_log(round(time.time()-start_time,2))
